...
describe("Lobbies", function () {
  ...
  describe("Leaving vs-AI Lobbies", function () {
    it("should close the lobby instead of promoting the AI to creator when the creator leaves after the AI accepted", async function () {
      const { creatorLobbies, creator, lobbies, singlePlayerMatch } =
        await loadFixture(deployLobbiesFixture);
      ...
      // AI accepts the reservation — joiner becomes SinglePlayerMatch
      await singlePlayerMatch.write.acceptMatch([1n]);

      let lobby = (await creatorLobbies.read.getLobby([1n])) as any;
      expect(lobby.players.joiner.toLowerCase()).to.equal(
        singlePlayerMatch.address.toLowerCase(),
      );
      expect(lobby.state.status).to.equal(LobbyStatus.FleetSelection);

      // Creator abandons the lobby — must close it outright, not hand
      // "creator" off to the AI orchestrator (which has no way to ever act
      // on a lobby as its creator)
      await creatorLobbies.write.leaveLobby([1n]);

      lobby = (await creatorLobbies.read.getLobby([1n])) as any;
      expect(lobby.basic.creator.toLowerCase()).to.equal(
        creator.account.address.toLowerCase(),
      );
      expect(lobby.players.joiner).to.equal(zeroAddress);
      ...
      // Fully removed from tracking, not reopened for anyone to join
      const openLobbyIds = await lobbies.read.getOpenLobbies();
      expect(openLobbyIds.length).to.equal(0);

      const creatorLobbyIds = await lobbies.read.getPlayerLobbies([
        creator.account.address,
      ]);
      expect(creatorLobbyIds.length).to.equal(0);

      // Creator's activeAILobbiesCount should be released too
      const playerState = (await creatorLobbies.read.getPlayerState([
        creator.account.address,
      ])) as unknown as PlayerLobbyState;
    ...
    it("should still promote a human joiner to creator when the creator leaves a PvP lobby", async function () {
      const { creatorLobbies, joinerLobbies, creator, joiner } =
        await loadFixture(deployLobbiesFixture);
      ...
      const lobby = (await creatorLobbies.read.getLobby([1n])) as any;
      expect(lobby.basic.creator.toLowerCase()).to.equal(
        joiner.account.address.toLowerCase(),
      );
      expect(lobby.players.joiner).to.equal(zeroAddress);
  ...
  describe("vs-AI Lobby Timeout Does Not Grief Other Players", function () {
    it("should not apply kick-penalty state to the shared AI orchestrator address on timeoutJoiner", async function () {
      const {
        creatorLobbies,
        otherLobbies,
        lobbies,
      ...
      // AI accepts, but is never given the chance to build its fleet
      await singlePlayerMatch.write.acceptMatch([1n]);

      await hre.network.provider.send("evm_increaseTime", [301]);
      await creatorLobbies.write.timeoutJoiner([1n]);

      // The shared AI orchestrator address must not have accrued a kick
      // penalty — otherwise this single timeout would lock every player
      // out of accepting an AI match for the next hour (and every repeat
      // compounds the lockout further, forever, with no reset path)
      const aiState = (await lobbies.read.getPlayerState([
        singlePlayerMatch.address,
      ])) as unknown as PlayerLobbyState;
      expect(aiState.kickCount).to.equal(0n);

      // Prove the griefing vector is actually closed: a different player
      // can immediately reserve + have the AI accept another lobby
      await otherLobbies.write.createLobby([
        1000n,
        300n,
        true,
      ...
      await expect(singlePlayerMatch.write.acceptMatch([2n])).to.not.be
        .rejected;

      const lobby2 = (await lobbies.read.getLobby([2n])) as any;
      expect(lobby2.players.joiner.toLowerCase()).to.equal(
        singlePlayerMatch.address.toLowerCase(),
      );
    });
